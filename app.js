...
async function loginGerente() {
  ...
  try {
    ...
    console.log('Gerentes retornados:', JSON.stringify(rows));
...
async function gesVerificarNome() {
  ...
  try {
    ...
    console.log('Gestores retornados:', JSON.stringify(all));
    ...
    if (!user) {
      ...
      console.log('Nomes disponíveis:', nomes);
...
async function loadMembros(membros) {
  ...
  for (var i = 0; i < membros.length; i++) {
    var m = membros[i];
    var avs = await dbGet('avaliacoes', 'membro_id=eq.' + m.id + '&select=*');
    var av = {};
    ...
    S.time.push({ id:m.id,nome:m.nome,mat:m.matricula,cargo:m.cargo,conta:m.conta||'',adm:m.adm||'',cadencia:m.cadencia||'Mensal',gestorId:m.gestor_id,avaliacoes:av });
  }
}
...
function calcScore(m, period) {
  ...
  var meses = Math.floor((new Date() - new Date(m.adm||'2024-01-01'))/(1000*60*60*24*30));
  var box = 'Estável';
  ...
  return { final:final, pot:pot, box:box, qual:qual, pen:pen, kpiScore:kpiScore, presScore:presScore, meses:meses };
}

function bbadge(b){ return {Alerta:'b-r',Excedente:'b-g',Atípico:'b-n',Promissor:'b-a'}[b]||'b-gr'; }
...
function skSet(mid, sid, val) {
  ...
  // Não rerender o select — só atualiza o score
  updateScoreLive(mid);
}
...
function updateScoreLive(mid) {
  ...
  el.innerHTML = '<div style="text-align:center;padding-right:1.5rem">'
    ...
    + '</div><div style="font-size:11px;color:var(--ts)">Pot: <b>'+sc.pot+'%</b> · Pen: <b style="color:var(--rd)">-'+sc.pen+'pts</b> · KPIs: <b>'+sc.kpiScore+'%</b> · Pres: <b>'+sc.presScore+'%</b></div></div>';
}
...
function pgAvaliacao(ct) {
  ...
  var sc = calcScore(m, S.selPeriod);
  var sk = m.cargo === 'Analista Jr' ? SK_JR : SK_SSR;
  var periods=(function(){var now=new Date(),list=[];for(var i=0;i<6;i++){var d=new Date(now.getFullYear(),now.getMonth()+i,1);list.push(d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-Mensal');}return list;})();
  var presOpts = {
  ...
  var scoreHtml = sc ? '<div id="score-live" style="display:flex;align-items:center;gap:1.5rem;padding:1rem;background:var(--gl);border-radius:var(--rm);border:1px solid var(--bd);flex-wrap:wrap;margin-bottom:.75rem">'
    +'<div style="text-align:center"><div style="font-size:34px;font-weight:700;color:'+(sc.final>=80?'var(--gr)':sc.final>=60?'var(--n)':'var(--rd)')+'">'+sc.final+'%</div><div style="font-size:11px;color:var(--ts)">Score final</div></div>'
    +'<div style="flex:1;min-width:160px"><div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:.4rem"><span class="badge '+bbadge(sc.box)+'">'+sc.box+'</span><span class="badge b-gr">'+sc.qual+'</span>'+(av.lancada?'<span class="badge b-g">✓ Enviado</span>':'<span class="badge b-a">Rascunho</span>')+'</div><div style="font-size:11px;color:var(--ts)">Pot: <b>'+sc.pot+'%</b> · Pen: <b style="color:var(--rd)">-'+sc.pen+'pts</b> · KPIs: <b>'+sc.kpiScore+'%</b> · Pres: <b>'+sc.presScore+'%</b></div></div>'
    +'</div>' : '';

  ct.innerHTML = '<div class="ph"><div class="pt">Avaliação</div><div class="pd">Score calculado em tempo real</div></div>'
    +'<div class="card">'
    +'<div class="fr">'
